...
function getKustomizeDeps {
  ...
  hack/run-in-gopath.sh godep get github.com/damienr74/kustomize/pkg/commands
...
function updateK8s {
  ...
  cp -r $KPATH/src/k8s.io/kubernetes/_output/local/go/src/github.com/damienr74/kustomize/k8sdeps \
  ...
  find $KPATH/src/k8s.io/kubernetes/staging/src/k8s.io/cli-runtime/pkg/kustomize/k8sdeps \
    -type f -name "*.go" | \
    xargs sed -i \
    's!github.com/damienr74/kustomize/k8sdeps!k8s.io/cli-runtime/pkg/kustomize/k8sdeps!'
...
function verify {
  # make sure in k8s.io/kubernetes/vendor/github.com/damienr74/kustomize
  # there is no internal package
  test 0 == $(ls $KPATH/src/k8s.io/kubernetes/vendor/github.com/damienr74/kustomize | grep “internal” | wc -l)
